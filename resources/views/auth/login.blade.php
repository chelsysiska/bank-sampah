    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-teal-50 to-green-50 px-6 py-12 relative overflow-hidden">
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 md:p-10 w-full max-w-xl relative border border-white/20">
            ...
            <div class="text-center mb-6 relative">
                ...
                <p class="text-gray-600 text-sm font-medium">Masuk ke akun Trash2Cash Anda</p>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <!-- Email / NIK -->
                <div class="group">
                    <label for="login" class="block text-sm font-bold text-gray-700 mb-1 tracking-wide">
                        Email atau NIK
                    </label>
                    <div class="relative">
                        ...
                        <input id="login" 
                               ...
                               value="{{ old('login') }}" 
                               required autofocus
                               class="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-200 focus:border-green-500 focus:ring-4 focus:ring-green-100 transition-all placeholder-gray-400 text-gray-900 bg-white/50 backdrop-blur-sm font-medium" 
                               placeholder="Masukkan email atau NIK" />
                    </div>
                    <x-input-error :messages="$errors->get('login')" class="mt-1 text-sm text-red-600 font-medium" />
                </div>
                ...
                <div class="group">
                    ...
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="fas fa-lock text-gray-400 group-focus-within:text-green-500 transition-colors"></i>
                        </div>
                        <input id="password" 
                               type="password" 
                               name="password" 
                ...
                <div class="flex items-center justify-between pt-2">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer">
                        <input id="remember_me" type="checkbox" name="remember"
                               class="w-5 h-5 rounded-lg border-2 border-gray-300 text-green-600 focus:ring-green-500 cursor-pointer">
                        <span class="ml-2 text-sm text-gray-700 font-semibold">Ingat saya</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}"
                           class="text-sm font-bold text-green-600 hover:text-green-700 hover:underline underline-offset-4">
                            Lupa password?
                        </a>
                    @endif
                </div>
                ...
                <!-- Submit -->
                <button type="submit"
                    class="w-full py-3.5 rounded-xl bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 text-white font-bold text-base shadow-xl hover:shadow-2xl transform hover:scale-[1.02] active:scale-[0.98] transition-all relative overflow-hidden group">
            ...
            <div class="text-center">
                ...
                <a href="{{ route('register') }}" 
                   class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-white border-2 border-green-500 text-green-600 font-bold text-sm hover:bg-green-50 transform hover:scale-[1.02] transition-all shadow-md hover:shadow-lg">
