...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden"> 
    <!-- Header --> 
     <div class="bg-gradient-to-r from-green-600 to-emerald-500 px-6 py-4 flex flex-col md:flex-row justify-between items-center"> 
        ...
        <div class="flex flex-col md:flex-row items-center gap-3 mt-3 md:mt-0"> 
            <form action="{{ route('petugas.laporan.kirim') }}" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row items-center gap-2" id="laporanForm"> 
                @csrf 
                <div class="flex flex-col"> 
                    ...
                    <!-- ✅ Info File & Preview -->
                    <p id="fileInfo" class="text-white text-sm italic mt-2">Belum ada bukti laporan.</p>
                    <img id="previewImage" src="#" alt="Preview" class="hidden mt-2 rounded-lg shadow-md max-h-32 border border-white/40"> 
                </div> 
                <button type="submit" id="submitButton" class="px-4 py-2 bg-white text-green-700 font-semibold rounded-lg shadow-md hover:bg-gray-100 transition-all" onclick="return confirm('Kirim atau perbarui laporan bulan ini ke admin?')"> 
                    <i class="fas fa-paper-plane mr-2"></i> {{ empty($laporanBulanIni?->bukti) ? 'Kirim Laporan' : 'Perbarui Laporan' }} 
                </button> 
            </form> 
        </div> 
    </div> 
    ...
    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50"> 
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-3 relative z-10 overflow-visible"> 
            <form method="GET" action="{{ route('petugas.setoran.index') }}" class="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl shadow-md overflow-visible flex-grow"> 
                <div class="flex items-center gap-2"> 
                    <label for="bulan" class="text-gray-700 font-medium">Bulan:</label> 
                    <select name="bulan" id="bulan" class="px-3 py-2 rounded-lg border border-gray-300 text-gray-700 bg-white focus:ring-2 focus:ring-green-400 focus:outline-none relative z-50"> 
                ...
                <div class="flex items-center gap-2"> 
                    ...
                    <select name="tahun" id="tahun" class="px-3 py-2 rounded-lg border border-gray-300 text-gray-700 bg-white focus:ring-2 focus:ring-green-400 focus:outline-none relative z-50"> 
                        <option value="">Semua</option> 
                ...
                <a href="{{ route('petugas.setoran.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition duration-200"> 
                    Reset 
                </a> 
        ...
        <table class="min-w-full border border-gray-200 rounded-lg text-sm bg-white"> 
            ...
            <tbody class="divide-y divide-gray-200 text-gray-800"> 
                ...
                <tr class="hover:bg-gray-50 transition-colors"> 
                    ...
                    <td class="px-4 py-3 font-semibold text-green-600"> Rp {{ number_format($setoran->total_harga, 0, ',', '.') }} </td> 
                    <td class="px-4 py-3"> 
                    ...
                    <td class="px-4 py-3 text-sm"> 
                        @if(!$setoran->is_reported) 
                        <form action="{{ route('petugas.setoran.destroy', $setoran->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus setoran ini?')" class="inline"> 
                            @csrf 
                            @method('DELETE') 
...
document.addEventListener('DOMContentLoaded', function() { 
    ...
    buktiInput.addEventListener('change', function() { 
        if (buktiInput.files.length > 0) { 
            ...
            // ✅ Tampilkan info file (nama + jenis)
